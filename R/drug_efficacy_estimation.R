...
TCGA_expr <- readRDS('rds/TCGA_expr.rds')
expr <- unlist(list(ccle_expr$expr,gdsc_expr$expr,PPTC_expr$expr,TCGA=list(TCGA_expr),HB=list(HB_expr)),recursive = F)
names(expr)[1:6] <- paste(rep(c('ccle','gdsc','PPTC'),each=2),names(expr)[1:6],sep = '_')
rm(ccle_expr,gdsc_expr,PPTC_expr,TCGA_expr,HB_expr)

#####

grd <- as.matrix(expand.grid(expression=names(expr)[1:6], sensitivity=names(PCC)))
for(i in tail(names(expr),2)) grd<- rbind(grd, c(i,'broad_solid'))


cl <- parallel::makeCluster(26)
